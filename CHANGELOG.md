#0.6.2
 * Added an action to witch what's shown on Status window
 * Mapped all the possible screens that the status window has
#0.6.1
 * Updated
 * Improved on how to circumvent the hardware bug
 * Removed blinking caused by thge device recieving too many packets too fast
 * Reduced number of packages sent to the device
#0.6.0
 * Improved on how to circumvent the hardware bug
 ...
 * Added a shell script to package the plug-in correctly
 #0.5.0
 * Circumvented a known bug in the hardware crash depending on the values in certain xzip positions
